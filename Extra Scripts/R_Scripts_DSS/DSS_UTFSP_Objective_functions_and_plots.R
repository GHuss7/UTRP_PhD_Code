...
# Load Libraries -------
libNames <- c( "ggplot2", "igraph","png","plotly","tikzDevice")
...
if(FALSE){ # extra code for manual input
R_routes = as.list(NULL)
R_routes[[1]] <- c(1,2,3,6,15,7,10,11)
R_routes[[2]] <- c(12,11,13,14,10,7,15,9)
R_routes[[3]] <- c(1,2,5,4,6,8,10,11)
R_routes[[4]] <- c(1,2,3,6,8,10,13,11)
R_routes[[5]] <- c(1,2,4,12,11,10,14,13)
R_routes[[6]] <- c(1,2,5,4,6,8,15,7)

R_max = as.list(NULL)
R_max[[1]] <- c(1,2,3,6,15,7,10,11)
R_max[[2]] <- c(12,11,13,14,10,7,15,9)
R_max[[3]] <- c(1,2,5,4,6,8,10,11)
R_max[[4]] <- c(1,2,3,6,8,10,13,11)
R_max[[5]] <- c(1,2,4,12,11,10,14,13)
R_max[[6]] <- c(1,2,5,4,6,8,15,7)
}
...
F_x = c(1/7, 1/7, 1/18, 1/5, 1/5, 1/20)
...
# Average expected travel time per passenger
sum(volumes_links*mx_C_a)/sum(mx_D_a)

# Number of buses used
sum(2*F_x*calculateRouteLengths4(distMatrix = S,routes = R_routes))

f3_avg_expected_travel_time(S,mx_D_a,R_routes,F_x)

f4_total_buses_required(S,R_routes, F_x)

f2_averageTravelTime(S,mx_D_a,R_routes)


# X.) Visualise the graph -----

if (T) { # indicate if you want visualisations
...
#plot(G10)
#customGraphPlot2(G10,plot_coords)
#customGraphPlot3(G10)

# Visualise flow volumes of network
#H10 <- createGraph2(volumes_links)
#tkp <- tkplot(H10,label.color = "darkblue", vertex.color = "gold",
              #labels = TRUE, edge.labels=TRUE)

#plot_coords_h <- tkplot.getcoords(tkp, norm = FALSE)

#plot(H10)
#customGraphPlot2(H10,plot_coords_h)
#customGraphPlot3(H10)
...
# Get the plot coords for the bigger graph
...
E(g)$width <- 2 # Assign widths to the graph
