...
"max_accepts" : 5, # maximum number of accepted moves per epoch
...
"Temp" : 20000,  # starting temperature and a geometric cooling schedule is used on it
"Cooling_rate" : 0.97, # the geometric cooling rate
"Reheating_rate" : 1.02, # the geometric reheating rate
"Number_of_initial_solutions" : 10000, # sets the number of initial solutions to generate as starting position
...
# %% Directory functions
"""Use this function to get the name of the most recent results"""
...
if generate_initial_set:
    ...
    df_routes_R_initial_set = pd.read_csv("Overall_Pareto_test_set_for_GA.csv") 
...
#    
 # %% Saving Results per run
    ...
    path_results = path_parent_folder / ("Results/Results_"+parameters_input['Problem_name']+"/"+parameters_input['Problem_name']+"_"+stats_overall['execution_start_time'].strftime("%Y%m%d_%H%M%S")+" "+parameters_SA_routes['method'])
    ...
    json.dump(parameters_SA_routes, open(path_results_per_run / "parameters_SA_routes.json", "w"))
...
# %% Display and save results per run'''
    if True:   
        ...
        plt.show()
        plt.savefig(path_results_per_run / "Results.pdf", bbox_inches='tight')
...
df_overall_pareto_set = ga.group_pareto_fronts_from_model_runs(path_results, parameters_input).iloc[:,1:]
...
# manager.window.showMaximized()
# plt.show()
# plt.savefig(path_results / "Results_combined.png", bbox_inches='tight')
# manager.window.close()
...
if False:
    ...
    # Repair function
    routes_R = gf.repair_add_missing_from_terminal(routes_R, parameters_input["n"], mapping_adjacent)
    
    # Mutate
    candidate_routes_R = gf.mutate_routes_two_intertwine(routes_R, parameters_constraints, mapping_adjacent)        
    #gv.plotRouteSet2(mx_dist, candidate_routes_R, mx_coords)     
           
    # Test all 4 constraints        
    gf.test_all_four_constraints_debug(candidate_routes_R, parameters_constraints)
    ...
    gf.test_all_four_constraints_debug(child_i, parameters_constraints)
    ...
    routes_R = gf.convert_routes_str2list(routes_R)
    gv.plotRouteSet2(mx_dist, routes_R, mx_coords)
    gf.test_all_four_constraints_debug(routes_R, parameters_constraints)
    
    routes_R = gf.repair_add_missing_from_terminal(routes_R, parameters_input["n"], UTNDP_problem_1.mapping_adjacent)
    gv.plotRouteSet2(mx_dist, routes_R, mx_coords)
