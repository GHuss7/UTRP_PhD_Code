...
for results_folder_name in result_entries:  
    if os.path.isdir(path_to_main_folder / results_folder_name):
        ...
        for i in range(nr_of_runs):
            results_file_path = path_to_main_folder / results_folder_name / ("Run_"+str(i+1)) # sets the path
        ...
        if False:
            df_all_obtained_HV.to_csv(path_to_main_folder / results_folder_name / "Results_all_avg_HV.csv")
        
        df_runs_summary = pd.DataFrame()
        df_runs_summary["Mean_HV"] = df_all_obtained_HV["Mean"]
        # df_runs_summary.to_csv(path_to_main_folder / results_folder_name / "Results_all_runs_summary.csv")
        df_HV_description.to_csv(path_to_main_folder / results_folder_name / "Results_description_HV_no_outliers.csv")
        df_HV_description_outliers.to_csv(path_to_main_folder / results_folder_name / "Results_description_HV.csv")
        df_HV_description_outliers_included.to_csv(path_to_main_folder / results_folder_name / "Results_description_HV_with_outliers.csv")
        df_Total_iterations_description.to_csv(path_to_main_folder / results_folder_name / "Results_description_Tot_iter.csv")
        df_overall_results.to_csv(path_to_main_folder / results_folder_name / "Overall_results.csv")