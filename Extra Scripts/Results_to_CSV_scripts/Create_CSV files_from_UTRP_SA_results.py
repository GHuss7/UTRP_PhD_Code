...
prefix_for_each_csv_file = "UTRP_SA_ST"
...
parameters_list = []
parameters_max_length_list = [] # list for storing the max run lenghts per parameter
...
for results_folder_name in result_entries:  
    if os.path.isdir(path_to_main_folder / results_folder_name):
        ...
        if parameter_name in parameters_list:
            ...
            # print(f"{parameter_name}:{len(df_results_read)} vs {parameters_max_length_list[parameters_list.index(parameter_name)]}")
            
            if len(df_results_read) > parameters_max_length_list[parameters_list.index(parameter_name)]:
                parameters_max_length_list[parameters_list.index(parameter_name)] = len(df_results_read)
                
        else:
            # Creates the new dataframe
            parameters_list.append(parameter_name)
            df_results_read = pd.read_csv(f"{path_to_main_folder / results_folder_name}/Results_all_runs_summary.csv") 
            parameters_max_length_list.append(len(df_results_read))
...
for results_folder_name in result_entries:  
    if os.path.isdir(path_to_main_folder / results_folder_name):
        ...
        if parameter_name in parameters_list:
            ...
            if parameters_max_length_list[parameters_list.index(parameter_name)] != len(df_results_read):
                df_blank_to_append = pd.concat([pd.DataFrame([""], columns=['Mean_HV']) for i in range(parameters_max_length_list[parameters_list.index(parameter_name)] - len(df_results_read))], ignore_index=True)
